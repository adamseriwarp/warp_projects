...
def generate_colors(n_colors):
    """Generate distinct colors for CBSAs"""
    import colorsys
    colors = []
...
def create_dynamic_cbsa_map(data, zip_shapes):
    """Create a dynamic map with real-time color changes for CBSAs"""
    print("Creating dynamic CBSA map...")
    
    # Filter for CBSAs
    cbsa_data = data[data['Primary CBSA Name'].notna()]
    ...
    unique_cbsas = sorted(map_data['Primary CBSA Name'].unique())
    colors = generate_colors(len(unique_cbsas))
...
<html>
<head>
    <title>Dynamic CBSA Zip Code Analysis</title>
    <meta charset="utf-8" />
...
<body>
    ...
    <div class="legend">
        <h4>🗺️ CBSA Zip Code Analysis</h4>
        <p><strong>All {len(unique_cbsas)} CBSAs</strong></p>
        <p>• Click zip codes to unassign from CBSA</p>
    ...
    <script>
        ...
        function createPopupContent(feature) {{
            var props = feature.properties;
            var status = props.assigned ? 'Assigned to CBSA' : 'Unassigned';
            var buttonText = props.assigned ? '🗑️ Unassign from CBSA' : '✅ Already Unassigned';
            ...
            return `
                <div class="popup-content">
                    ...
                    <strong>CBSA:</strong> ${{props.cbsa_name || 'None'}}<br>
        ...
        function unassignZip(zipcode) {{
            ...
            alert('Zip code ' + zipcode + ' has been unassigned from its CBSA!\\nIt should now appear gray on the map.');
        }}
        ...
        function exportUnassignedZips() {{
            ...
            a.href = url;
            a.download = 'unassigned_cbsa_zip_codes_' + new Date().toISOString().slice(0,19).replace(/:/g, '-') + '.csv';
            a.click();
        ...
        function resetAllChanges() {{
            ...
            if (confirm('Reset all ' + unassignedZips.length + ' unassigned zip codes back to their original CBSA assignments?')) {{
                // Reset all zip codes
            ...
            var layer = L.geoJSON(feature, {{
                ...
                onEachFeature: function(feature, layer) {{
                    ...
                    layer.bindTooltip(
                        ...
                        ' | CBSA: ' + (feature.properties.cbsa_name || 'None').substring(0, 30) + 
        ...
        console.log('CBSA map loaded with', Object.keys(zipLayers).length, 'zip codes');
    </script>
</body>
</html>
"""
...
def main():
    """Main function to create and display the dynamic CBSA map"""
    try:
        # Load data
        merged_data, zip_shapes = load_and_prepare_data()
        
        # Create dynamic map
        html_content = create_dynamic_cbsa_map(merged_data, zip_shapes)
