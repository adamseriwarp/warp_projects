...
def load_serviced_zip_codes():
    """Load zip codes from LTL Rate File to determine which zip codes we service"""
    print("Loading serviced zip codes from LTL Rate File...")
    
    ltl_file = DATA_DIR / 'raw' / 'LTL Rate File 07-07-2025 - Zips.csv'
    
    try:
        ltl_data = pd.read_csv(ltl_file)
        print(f"LTL Rate File columns: {ltl_data.columns.tolist()}")
        
        # Clean zip codes to 5 digits
        ltl_data['Zip_clean'] = ltl_data['Zip'].astype(str).str.zfill(5)
        serviced_zips = ltl_data['Zip_clean'].unique().tolist()
        
        print(f"Found {len(serviced_zips)} unique serviced zip codes")
        print(f"Sample serviced zips: {serviced_zips[:10]}")
        
        return serviced_zips, ltl_data
        
    except Exception as e:
        print(f"Error loading LTL Rate File: {e}")
        print("Using empty list - no serviced zip codes found")
        return [], pd.DataFrame()

def identify_serviced_cbsas(serviced_zips, cbsa_mapping):
    """Identify CBSAs that contain at least one serviced zip code"""
    print("Identifying CBSAs with serviced zip codes...")
    
    # Filter CBSA mapping to only serviced zip codes
    serviced_cbsa_mapping = cbsa_mapping[cbsa_mapping['Zip Code_clean'].isin(serviced_zips)]
    
    # Get unique CBSAs where we have service
    serviced_cbsas = serviced_cbsa_mapping[serviced_cbsa_mapping['Primary CBSA Name'].notna()]['Primary CBSA Name'].unique().tolist()
    
    print(f"Found {len(serviced_cbsas)} CBSAs with serviced zip codes:")
    for i, cbsa in enumerate(serviced_cbsas[:10], 1):
        zip_count = len(serviced_cbsa_mapping[serviced_cbsa_mapping['Primary CBSA Name'] == cbsa])
        print(f"  {i}. {cbsa}: {zip_count} serviced zip codes")
    if len(serviced_cbsas) > 10:
        print(f"  ... and {len(serviced_cbsas) - 10} more CBSAs")
    
    return serviced_cbsas

def load_and_prepare_data():
    """Load and prepare the data for heat mapping"""
    print("Loading data...")
    
    # Load serviced zip codes first
    serviced_zips, ltl_data = load_serviced_zip_codes()
    ...
    # Identify CBSAs with serviced zip codes
    serviced_cbsas = identify_serviced_cbsas(serviced_zips, cbsa_mapping)
    ...
    return merged_data, zip_shapes, unassigned_zips_clean, serviced_cbsas
...
def calculate_weighted_cbsa_centroids(data, zip_shapes, serviced_cbsas):
    """Calculate quote-weighted centroids for each serviced CBSA"""
    print("Calculating quote-weighted CBSA centroids for serviced CBSAs...")

    # MODIFIED: Use serviced CBSAs instead of top 75 by population
    print(f"Using {len(serviced_cbsas)} serviced CBSAs instead of top 75 by population...")

    for i, cbsa in enumerate(serviced_cbsas[:10], 1):
        print(f"  {i}. {cbsa}")
    if len(serviced_cbsas) > 10:
        print(f"  ... and {len(serviced_cbsas) - 10} more")
    ...
    # Filter map data to only include serviced CBSAs (but keep all non-CBSA zip codes)
    map_data = map_data[
        (map_data['Primary CBSA Name'].isin(serviced_cbsas)) |
    ...
    print(f"Calculating weighted centroids for {cbsa_zips['Primary CBSA Name'].nunique()} serviced CBSAs...")
    ...
    print(f"Calculated weighted centroids for {len(cbsa_centroids_df)} serviced CBSAs")
    print(f"Quote range across serviced CBSAs: {cbsa_centroids_df['total_quotes'].min():,.0f} - {cbsa_centroids_df['total_quotes'].max():,.0f}")
...
<html>
<head>
    <title>Serviced CBSAs Color-Coded Map with Smart Corridors</title>
    <meta charset="utf-8" />
...
<body>
    ...
    <div class="controls">
        <h4>🎨 <span class="serviced-badge">SERVICED</span> CBSAs Color-Coded Map</h4>
        ...
        <div class="checkbox-container">
            ...
            <label for="showAllCBSA">Show all serviced CBSA zip codes</label>
        </div>
    ...
    html_content += f"""
        ...
        <div style="background: #e3f2fd; padding: 8px; border-radius: 4px; font-size: 12px; margin: 10px 0;">
            <strong>🎯 SERVICED CBSAs Features:</strong><br>
            ✅ Shows only CBSAs where we have serviced zip codes<br>
            ✅ Each CBSA has a unique color for easy distinction<br>
            ✅ Red color for non-CBSA zip codes<br>
            ✅ Export ALWAYS includes corridor zip codes (even if outside filters)<br>
        ...
        <button onclick="exportFilteredData()" style="width: 100%; padding: 10px; margin-top: 15px; background: #007cba; color: white; border: none; border-radius: 5px; cursor: pointer; font-weight: bold;">
            📥 Export Filtered + ALL Corridor Zip Codes
        </button>
    </div>

    <div class="legend">
        <h4>🎯 Serviced CBSAs Color-Coded Map</h4>
        <div><strong>CBSA-Assigned (Serviced):</strong></div>
        <div class="color-scale">
            ...
            <span>Each serviced CBSA has a unique color (no red)</span>
        </div>
    ...
    <script>
        ...
        var currentCorridorZipCodes = new Set(); // Track current corridor zip codes

        console.log('🎯 SERVICED CBSAs Map loaded with:');
        console.log('   -', allZipData.length, 'zip codes from serviced CBSAs');
        console.log('   -', cbsaCentroids.length, 'serviced CBSA centroids');
        console.log('   -', crossdockLocations.length, 'crossdocks');
        console.log('   -', allUsaData.length, 'USA zip codes for corridor analysis');"""

    # Add the rest of the JavaScript functionality
    html_content += """
    ...
    # Add the remaining JavaScript functions and close the HTML
    html_content += """
        ...
        function toggleSmartCorridors() {
            ...
            if (smartCorridorsVisible) {
                ...
                // Clear corridor zip codes tracking
                currentCorridorZipCodes.clear();
            } else {
                ...
                // Clear and update corridor zip codes tracking
                currentCorridorZipCodes.clear();
                corridorZips.forEach(zip => {
                    currentCorridorZipCodes.add(zip.properties.zipcode_clean);
                });
                ...
                if (corridorZips.length > 0) {
                    ...
                    console.log('Tracking', currentCorridorZipCodes.size, 'corridor zip codes for export');
                } else {
        ...
        function updateMapDisplay() {
            ...
            allZipData.forEach(function(zipData) {
                ...
                if (shouldShow) {
                    ...
                    var nearestCrossdock = findNearestCrossdock(
                        zipData.geometry.coordinates[0][0][1], // lat
                        zipData.geometry.coordinates[0][0][0]  // lon
                    );

                    var layer = L.geoJSON(zipData, {
                        style: {
                            fillColor: props.cbsa_color,
                            color: props.has_cbsa ? '#333' : '#FF0000',
                            weight: 1,
                            opacity: 0.8,
                            fillOpacity: 0.7
                        },
                        onEachFeature: function(feature, layer) {
                            var popupContent = `
                                <strong>Zip Code: ${feature.properties.zipcode_clean}</strong><br>
                                ...
                                <strong>Pickup:</strong> ${feature.properties.pickup_count} | <strong>Dropoff:</strong> ${feature.properties.dropoff_count}<br>
                                <hr style="margin: 5px 0;">
                            `;

                            if (feature.properties.has_cbsa) {
                                popupContent += `
                                    <strong>🎯 CBSA:</strong> ${feature.properties.cbsa_name}<br>
                                    <strong>CBSA Mean Quotes:</strong> ${feature.properties.cbsa_mean_quotes.toFixed(1)}<br>
                                `;
                            } else {
                                popupContent += `
                                    <strong>❌ Non-CBSA Zip Code</strong><br>
                                    <strong>Distance to nearest CBSA:</strong> ${feature.properties.distance_to_cbsa.toFixed(1)} miles<br>
                                    <strong>Nearest CBSA:</strong> ${feature.properties.nearest_cbsa_name}<br>
                                    <strong>Quote Performance:</strong> ${feature.properties.std_dev_from_cbsa_mean.toFixed(2)} std dev from CBSA mean<br>
                                    <strong>CBSA Mean Quotes:</strong> ${feature.properties.cbsa_mean_quotes.toFixed(1)}<br>
                                `;
                            }

                            if (nearestCrossdock.crossdock) {
                                popupContent += `
                                    <hr style="margin: 5px 0;">
                                    <strong>🏭 Nearest Crossdock:</strong> ${nearestCrossdock.crossdock.name}<br>
                                    <strong>Distance:</strong> ${nearestCrossdock.distance.toFixed(1)} miles
                                `;
                            }
            ...
            document.getElementById('filterStats').innerHTML = `
                ...
                • Non-CBSA zip codes: ${nonCbsaCount.toLocaleString()}<br>
                • Total displayed: ${totalDisplayed.toLocaleString()}<br>
                • Distance filter: ≤ ${maxDistance} miles<br>
                • Performance filter: ≥ ${minStdDev} std dev${stdDevFilterEnabled ? ' (enabled)' : ' (disabled)'}
            `;

            // Update centroids
            updateCentroids(showCentroids);

            // Update crossdocks
            updateCrossdocks(showCrossdocks);
        }

        // Update CBSA centroids display
        function updateCentroids(show) {
            ...
            if (show) {
                cbsaCentroids.forEach(function(centroid) {
                    var marker = L.circleMarker([centroid.lat, centroid.lon], {
                        radius: Math.max(5, Math.min(15, Math.sqrt(centroid.total_quotes / 1000))),
                        fillColor: '#FF0000',
                        color: '#800000',
                        weight: 2,
                        opacity: 1,
                    ...
                    marker.bindPopup(`
                        <strong>🎯 CBSA Centroid</strong><br>
                    ...
                    marker.bindTooltip(
                        '🎯 ' + centroid.cbsa_name + ' | ' + centroid.total_quotes.toLocaleString() + ' quotes',
                        { permanent: false, direction: 'top' }
                    );
        ...
        // Update crossdocks display
        function updateCrossdocks(show) {
            // Clear existing crossdocks
            ...
            if (show) {
                crossdockLocations.forEach(function(crossdock) {
                    ...
                    marker.bindPopup(`
                        <strong>🏭 Crossdock Location</strong><br>
        ...
        function exportFilteredData() {
            ...
            // Start with filtered data from main dataset
            var filteredData = allZipData.filter(function(zipData) {
                var props = zipData.properties;

                // ALWAYS include corridor zip codes, regardless of other filters
                if (currentCorridorZipCodes.has(props.zipcode_clean)) {
                    return true;
                }

                // Apply normal filtering for non-corridor zip codes
                if (props.has_cbsa && showCBSA) {
                    return true;
                } else if (!props.has_cbsa && showNonCBSA) {
                    if (props.distance_to_cbsa <= maxDistance) {
            ...
            // Add any corridor zip codes from allUsaData that aren't in the main dataset
            var mainDatasetZipCodes = new Set(allZipData.map(zip => zip.properties.zipcode_clean));

            currentCorridorZipCodes.forEach(function(corridorZipCode) {
                // If this corridor zip code isn't in our main dataset, find it in allUsaData
                if (!mainDatasetZipCodes.has(corridorZipCode)) {
                    var corridorZip = allUsaData.find(zip => zip.properties.zipcode_clean === corridorZipCode);
                    if (corridorZip) {
                        // Create a compatible data structure for export
                        var compatibleZip = {
                            properties: {
                                zipcode_clean: corridorZip.properties.zipcode_clean,
                                city: 'Unknown', // allUsaData doesn't have city info
                                state: 'Unknown', // allUsaData doesn't have state info
                                total_quotes: corridorZip.properties.total_quotes || 0,
                                pickup_count: 0, // Not available in allUsaData
                                dropoff_count: 0, // Not available in allUsaData
                                population: 0, // Not available in allUsaData
                                has_cbsa: corridorZip.properties.has_cbsa,
                                cbsa_name: corridorZip.properties.cbsa_name || '',
                                distance_to_cbsa: 0, // Will need to calculate if needed
                                nearest_cbsa_zip: '',
                                nearest_cbsa_name: '',
                                std_dev_from_cbsa_mean: 0,
                                quote_percentile_in_nearest_cbsa: 0,
                                assigned_cbsa: corridorZip.properties.cbsa_name || '',
                                closest_cbsa: corridorZip.properties.cbsa_name || '',
                                cbsa_mean_quotes: 0
                            },
                            geometry: corridorZip.geometry
                        };
                        filteredData.push(compatibleZip);
                    }
                }
            });

            // Prepare CSV data
            var csvContent = "Zipcode,City,State,Total_Quotes,Pickup_Count,Dropoff_Count,Population,Has_CBSA,CBSA_Name,Distance_to_CBSA,Nearest_CBSA_Zip,Nearest_CBSA_Name,Std_Dev_from_CBSA_Mean,Quote_Percentile_in_Nearest_CBSA,Assigned_CBSA,Closest_CBSA,CBSA_Mean_Quotes,Is_Corridor_Zip\\n";

            filteredData.forEach(function(zipData) {
                var props = zipData.properties;
                var nearestCrossdock = findNearestCrossdock(
                    zipData.geometry.coordinates[0][0][1],
                    zipData.geometry.coordinates[0][0][0]
                );

                // Check if this zip code is currently a corridor zip code
                var isCorridorZip = currentCorridorZipCodes.has(props.zipcode_clean);

                csvContent += [
                    props.zipcode_clean,
                    '"' + (props.city || '').replace(/"/g, '""') + '"',
                    props.state || '',
                    props.total_quotes || 0,
                    props.pickup_count || 0,
                    props.dropoff_count || 0,
                    props.population || 0,
                    props.has_cbsa ? 'Yes' : 'No',
                    '"' + (props.cbsa_name || '').replace(/"/g, '""') + '"',
                    (props.distance_to_cbsa || 0).toFixed(2),
                    props.nearest_cbsa_zip || '',
                    '"' + (props.nearest_cbsa_name || '').replace(/"/g, '""') + '"',
                    (props.std_dev_from_cbsa_mean || 0).toFixed(3),
                    (props.quote_percentile_in_nearest_cbsa || 0).toFixed(1),
                    '"' + (props.assigned_cbsa || '').replace(/"/g, '""') + '"',
                    '"' + (props.closest_cbsa || '').replace(/"/g, '""') + '"',
                    (props.cbsa_mean_quotes || 0).toFixed(2),
                    isCorridorZip ? 'Yes' : 'No'
                ].join(',') + '\\n';
            });
            ...
            link.setAttribute('download', 'serviced_cbsas_filtered_zip_codes_' + new Date().toISOString().slice(0,19).replace(/:/g, '-') + '.csv');
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);

            var corridorCount = filteredData.filter(zip => currentCorridorZipCodes.has(zip.properties.zipcode_clean)).length;
            console.log('Exported', filteredData.length, 'zip codes to CSV (including', corridorCount, 'corridor zip codes)');
        }
        ...
        document.getElementById('showCentroids').addEventListener('change', function() {
            updateCentroids(this.checked);
        });
        document.getElementById('showCrossdocks').addEventListener('change', function() {
            updateCrossdocks(this.checked);
        });
        ...
        document.getElementById('maxCorridorDistance').addEventListener('input', function() {
            ...
            if (smartCorridorsVisible) {
                ...
                smartCorridorLayers = [];
                smartCorridorsVisible = false;
                currentCorridorZipCodes.clear(); // Clear tracking

                // Show new corridors with updated distance
                toggleSmartCorridors();
            }
        });
        ...
        document.getElementById('baseCorridorWidth').addEventListener('input', function() {
            ...
            if (smartCorridorsVisible) {
                ...
                // Show new corridors with updated width
                toggleSmartCorridors();
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('🎯 Serviced CBSAs map initialized');
            updateMapDisplay();
        });

        // Initialize map display immediately
        updateMapDisplay();
    </script>
</body>
</html>"""
...
def main():
    ...
    print("🎯 Starting Serviced CBSAs Color-Coded Corridor Map Analysis...")
    print("=" * 80)

    try:
        # Load and prepare data
        merged_data, zip_shapes, unassigned_zips, serviced_cbsas = load_and_prepare_data()

        if not serviced_cbsas:
            print("❌ No serviced CBSAs found! Please check your LTL Rate File and CBSA mapping.")
            return

        # Calculate weighted centroids for serviced CBSAs
        map_data, cbsa_centroids_df = calculate_weighted_cbsa_centroids(merged_data, zip_shapes, serviced_cbsas)
        ...
        zip_features, cbsa_centroid_features = create_cbsa_proximity_map(
            map_data, cbsa_centroids_df, unassigned_zips, crossdock_locations, all_usa_data_js
        )
        ...
        # Save the HTML file
        output_file = BASE_DIR / 'serviced_cbsas_color_coded_corridor_map.html'
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(html_content)

        print(f"✅ Map created successfully: {output_file}")
        print(f"📊 Summary:")
        print(f"   - {len(serviced_cbsas)} serviced CBSAs")
        print(f"   - {len(zip_features)} zip codes displayed")
        ...
        try:
            ...
            print(f"Please open this file manually: {output_file}")
